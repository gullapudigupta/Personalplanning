...
def createPromt(df_user, df_dest):
    ...
    prompt_text = prompt_initial_text+user_intro_text
...
def getUser(userId):
    ...
    df_dest = wr.athena.read_sql_query(
        sql="SELECT * FROM destination_data_csv where activities_available like '%" + activities  + "%'",
...
def get_bedrock_chain(user_id):
    ...
    prompt_template = getUser(user_id)
